...
import styles from './TruckNote.module.css';
...
const BoatNote = () => {
    ...
    const handleScanTagCheckByDO = (event, do_num) => {
        ...
        if (event.key === 'Enter') {
            if (do_group_list !== '') {
                API.get(`API_QuantityMove/data.php?load=SearchTagDetailCheckByDOBoat&tag_id=${event.target.value}&do_num=${do_group_list.do_group_list}`)
                    .then(res => {
                        const items = res.data
                        if (res.data.length > 0) {
                            setQtyMoveList(qtyMoveList => [...qtyMoveList,
                                            ...
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item lg={9} >
                                                    <DataTable
                                                        ...
                                                        search={false}
                                                        sorting
                                                        rowStyle={qtyMoveRowStyle}
                                                        editable={{
                                                            onRowDelete: oldData =>
                                                                new Promise((resolve) => {
                                                                    setTimeout(() => {
                                                                        const newData = qtyMoveList.filter((item) => item.id !== oldData.id);
                                                            ...
                                                            onRowUpdate: (newData, oldData) =>
                                                                new Promise((resolve) => {
                                                                    setTimeout(() => {
                                                                        const dataUpdate = qtyMoveList.map((item) => (
                ...
                <Grid item lg={12}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} className={styles.warningRow}>
                            <label className={styles.warning}>***ใช้สำหรับย้ายขึ้นรถเท่านั้น</label>
                        </Grid>
