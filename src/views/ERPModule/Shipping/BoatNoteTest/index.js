...
const BoatNoteTest = () => {
    ...
    const handleOpenModalCreateDoGroup = () => {
        APITEST.get(`API_QuantityMove/data.php?load=STS_list_of_do_group`)
            .then(res => {
    ...
    useEffect(() => {
        APITEST.get(`API_QuantityMove/data.php?load=STS_qty_move_hrd`)
            .then(res => {
    ...
    const handleClickSelectDoc = (row) => {
        ...
        APITEST.get(`API_QuantityMove/data.php?load=STS_qty_move_line&doc_num=${row.doc_num}`)
            .then(res => {
                setSTS_qty_move_line(res.data)
            })

        APITEST.get(`API_QuantityMove/data.php?load=checkItemLotLoc&loc=${row.loc}`)
            .then(res => {
    ...
    const handleScanTag = (event) => {
        if (event.key === 'Enter') {
            APITEST.get(`API_QuantityMove/data.php?load=SearchTagDetail&tag_id=${event.target.value}`)
                .then(res => {
    ...
    const handleScanTagCheckByDO = (event, do_num) => {
        if (event.key === 'Enter') {
            if (do_group_list !== '') {
                APITEST.get(`API_QuantityMove/data.php?load=SearchTagDetailCheckByDO&tag_id=${event.target.value}&do_num=${do_group_list}`)
                    .then(res => {
                        const items = res.data
                        if (res.data.length > 0) {
                            setQtyMoveList(qtyMoveList => [...qtyMoveList,
    ...
    const saveDocument = (values, qtyMoveList) => {
        ...
        if (!values.loc || qtyMoveList.length === 0) {
            ...
            APITEST.get(`API_QuantityMove/data.php?load=moveqty_create_hdr&toLoc=${values.loc}&w_c=${values.wc}&doc_type=${values.doc_type}&do_num=${values.do_num}&boatList=${values.boatList}&destination=${destination}`)
                .then(res => {
                    ...
                    if (qtyMoveList.length > 0) {
                        ...
                        const timer = setInterval(() => {
                            APITEST.get(`API_QuantityMove/data.php?load=moveqty_create_line&docnum=${moveqty_hdr.doc_num}&docline=${i}&tagnum=${qtyMoveList[i - 1].id}&toLoc=${values.loc}&boatPosition=${qtyMoveList[i - 1].boat_position}`)
                                .then(res => {
    ...
    const CRUDfn = async (load, values) => {
        await APITEST.get(`API_QuantityMove/data.php`, {
            params: {
                load: load,
                id: values.id,
            ...
            <ModalManagementFullPage
                ...
                modalDetail={
                    <div style={{ maxWidth: '100%' }}>
                        <MaterialTable
                            icons={tableIcons}
                            title="กลุ่มใบ DO ที่ต้องการขนส่งtest"
                            columns={[
                            ...
                            onRowClick={(event, rowData) => {
                                ...
                                APITEST.get(`API_QuantityMove/data.php?load=SelectDOList&do_num=${rowData.do_group_list}`)
                                    .then(res => {
