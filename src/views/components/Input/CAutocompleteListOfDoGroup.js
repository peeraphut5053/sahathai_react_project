...
export default function CAutocompleteListOfDoGroup(props) {
    ...
    React.useEffect(() => {
        ...
        (async () => {
            let response;
            if (window.location.host == '172.18.1.194:5000' || window.location.host == 'localhost:3100') {
                response = await fetch('http://172.18.1.194/sts_web_center/module/API_QuantityMove/data.php?load=STS_list_of_do_group');
            } else {
                response = await fetch('http://61.90.156.165/sts_web_center/module/API_QuantityMove/data.php?load=STS_list_of_do_group');
            }
